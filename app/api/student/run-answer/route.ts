...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    await insertServerEvent({ session_id: sessionId, profile_id: profile.profile_id, task_id: taskId, event_type: "sql_run", event_value: "run_button_click", duration_from_start: calculateDurationFromStart(session.started_at), metadata_json: { answer_length: answerText.length } }, userClient);
    await insertServerAttempt({ session_id: sessionId, profile_id: profile.profile_id, task_id: taskId, attempt_type: "run", answer_text: answerText, answer_json: answerJson, is_correct: result.is_correct, score: result.score, error_type: result.error_type, error_message: result.error_message, execution_time_ms: executionTimeMs }, userClient);
    await insertServerEvent({ session_id: sessionId, profile_id: profile.profile_id, task_id: taskId, event_type: result.is_correct ? "sql_success" : "sql_error", event_value: result.is_correct ? "answer_matched_expected" : "answer_mismatch", duration_from_start: calculateDurationFromStart(session.started_at) }, userClient);
