...
export default function TeacherExamSetsPage() {
  ...
  useEffect(() => {
    async function loadSets() {
      ...
      const response = await fetch("/api/teacher/assignmentsets?family=exam", {
      ...
      if (!response.ok) {
        ...
        else setErrorMessage(json.error ?? text ?? "Failed to load exam sets.");
        setLoading(false);
        return;
      }
  ...
  async function toggleSetStatus(setId: string, nextActive: boolean) {
    ...
    if (response.ok) {
      ...
      const text = await response.text();
      alert(safeJsonParse(text).error ?? "Failed to update exam set.");
    }
    setUpdatingId(null);
  }

  if (loading) {
    return <div className="min-h-screen bg-[#FFF7ED] flex items-center justify-center text-sm text-[#64748B]">Loading exam sets...</div>;
  }
    ...
    <div className="min-h-screen bg-[#FFF7ED]">
      <header className="bg-white border-b border-[#FED7AA] px-6 py-3">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <span className="text-xs font-semibold text-[#F37021]">Exam Sets by teacher</span>
        </div>
      </header>
      ...
      <main className="max-w-6xl mx-auto px-4 py-8 space-y-6">
        <section className="flex flex-col lg:flex-row lg:items-end gap-4 justify-between">
          <div>
            <h1 className="text-2xl font-bold text-[#0F172A]">Exam Sets</h1>
            <p className="text-sm text-[#64748B] mt-1">Exam batches created by this teacher.</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link href="/teacher/exams/new" className="px-4 py-2 rounded-xl bg-[#F37021] hover:bg-[#C2410C] text-white text-sm font-semibold">
              New Exam Set
            </Link>
            <Link href="/teacher/exams/create" className="px-4 py-2 rounded-xl border border-[#F37021] text-[#F37021] bg-white hover:bg-[#FFF7ED] text-sm font-semibold">
              New Exam
            </Link>
            <Link href="/teacher/exams/list" className="px-4 py-2 rounded-xl border border-[#FED7AA] text-[#0F172A] bg-white hover:border-[#F37021] text-sm font-semibold">
              Exam List
            </Link>
          </div>
        </section>
        ...
        <section className="bg-white border border-[#FED7AA] rounded-2xl p-4 flex flex-col lg:flex-row gap-3">
          <input
            ...
            placeholder="Search by exam set code or name"
              ...
              <article key={set.batch_id} className="bg-white border border-[#FED7AA] rounded-2xl p-5 shadow-sm">
                <div className="flex flex-col lg:flex-row lg:items-start gap-4">
                  <div className="flex-1 min-w-0">
                    ...
                    <h2 className="text-base font-bold text-[#0F172A]">{set.batch_name ?? "Untitled exam set"}</h2>
                    ...
                    <div className="flex flex-wrap gap-4 mt-4 text-xs text-[#64748B]">
                      <span>{set.task_count} exams</span>
                  ...
                  <div className="flex flex-col items-start lg:items-end gap-3">
                    ...
                    <div className="flex items-center gap-3">
                      <Link
                        href={`/teacher/exams/${set.batch_id}`}
                        aria-label="Edit exam set"
                        title="Edit"
...
function SetActiveSwitch({ active, disabled, onToggle }: { active: boolean; disabled: boolean; onToggle: (active: boolean) => void }) {
  return (
    <button
      type="button"
      aria-pressed={active}
      aria-label="Toggle exam set active status"
      disabled={disabled}
...
function safeJsonParse(text: string): { error?: string; assignment_sets?: ExamSet[] } {
  try {
